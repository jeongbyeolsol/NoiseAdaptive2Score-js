...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'unet':
        net = UNet(input_nc, output_nc, base_channels=ngf)
    elif netG == 'dncnn':
        net = DnCNN()     
    elif netG == 'resnet':
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, base_channels=48):
        """Initializes U-Net."""

        super(UNet, self).__init__()
        if base_channels < 3:
            raise ValueError('base_channels must be at least 3')
        c = base_channels
        decoder_c = 2 * c
        final_c1 = max(1, 4 * c // 3)
        final_c2 = max(1, 2 * c // 3)
        ...
        self._block3 = nn.Sequential(
            nn.Conv2d(c, c, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(c, c, 3, stride=2, padding=1, output_padding=1))
            #nn.Upsample(scale_factor=2, mode='nearest'))

        # Layers: dec_conv5a, dec_conv5b, upsample4
        self._block4 = nn.Sequential(
            nn.Conv2d(2 * c, decoder_c, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(decoder_c, decoder_c, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(decoder_c, decoder_c, 3, stride=2, padding=1, output_padding=1))
            #nn.Upsample(scale_factor=2, mode='nearest'))

        # Layers: dec_deconv(i)a, dec_deconv(i)b, upsample(i-1); i=4..2
        self._block5 = nn.Sequential(
            nn.Conv2d(3 * c, decoder_c, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(decoder_c, decoder_c, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(decoder_c, decoder_c, 3, stride=2, padding=1, output_padding=1))
        ...
        self._block6 = nn.Sequential(
            nn.Conv2d(decoder_c + in_channels, final_c1, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(final_c1, final_c2, 3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(final_c2, out_channels, 3, stride=1, padding=1))
